...
export const useSiteAssessmentDetails = (assessmentId: string | undefined) => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      console.log('[useSiteAssessmentDetails] Fetching assessment details:', { assessmentId, userId: user?.id });
    ...
    retry: (failureCount, error) => {
      console.log('[useSiteAssessmentDetails] Retry attempt:', failureCount, 'Error:', error.message);
