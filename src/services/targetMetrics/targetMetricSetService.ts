...
// Helper function to infer enabled sections from existing metrics
...
export const getTargetMetricSetById = async (id: string, userId: string): Promise<TargetMetricSet | null> => {
  ...
  let finalEnabledSections = enabledSections;
  if (enabledSections.length === 0 && data?.user_custom_metrics_settings && data.user_custom_metrics_settings.length > 0) {
    console.log('[getTargetMetricSetById] No enabled sections found, inferring from existing metrics');
    const inferredSections = inferEnabledSectionsFromMetrics(data.user_custom_metrics_settings);
    finalEnabledSections = inferredSections;
    console.log('[getTargetMetricSetById] Inferred enabled sections:', inferredSections);
  }
  ...
  return {
    ...
    enabled_optional_sections: finalEnabledSections
  };
};
