...
serve(async (req: Request) => {
  ...
  try {
    ...
    const prompt = `
      You are a real estate strategist who uses data to identify promising site opportunities and flag potential risks. Your job is to write a clear, concise, and plain-spoken executive summary of a site assessment that helps brand stakeholders quickly understand whether a site is worth pursuing—and why.

      Follow these instructions:

      1. Begin by naming the exact Target Metrics Set used.

      2. Lead with the most important insight—what makes this site stand out (positively or negatively)?

      3. List the specific Target Metrics referenced in your analysis.

      4. Highlight strong performance areas and explain how the brand can capitalize on them. Examples:
      ...
      5. Call out any poor performance or high-risk areas. For each, give a specific mitigation strategy. (e.g., poor trade area = invest in hyperlocal marketing or brand education)

      6. Do not mention average or unremarkable sections.

      7. Keep your tone conservative. Avoid exaggeration, especially on borderline results.

      8. Limit your response to 150 words so it can be read at a glance.

      9. End with this disclaimer:
    ...
    const completion = await openai.createChatCompletion({
      model: "gpt-4o-mini",
      messages: [
        { role: "system", content: "You are a real estate strategist providing executive summaries for site assessments. Follow the exact format and instructions provided. Keep responses under 150 words and always end with the specified disclaimer." },
